...
The supplied geometric mark is now the primary brand asset at `public/brand/sarv-logo.png`.
...
The product name is **Sarv**. All visible branding and local project identifiers use `Sarv`/`sarv`.

Important: do not rename `VITE_SUPABASE_URL` or `VITE_SUPABASE_ANON_KEY` values to match the brand. They are Supabase project credentials generated by Supabase.
